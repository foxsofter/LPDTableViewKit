...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *scrollToCellBarButtonItem =
    [[UIBarButtonItem alloc] initWithTitle:@"scroll" style:UIBarButtonItemStylePlain target:self action:@selector(scrollToCell)];
...
- (void)scrollToCell {
    ...
    NSArray *rows = [[sections objectAtIndex:sections.count-1] rows];
//    NSIndexPath *indexPath = [NSIndexPath indexPathForRow:rows.count-1 inSection:sections.count-1];
    NSIndexPath *indexPath = [NSIndexPath indexPathForRow:10 inSection:0];
    [self.tableViewModel scrollToCellAtIndexPath:indexPath atScrollPosition:UITableViewScrollPositionTop animated:YES];
}

- (void)scrollToNearestSelectedCell {
    [self.tableViewModel scrollToNearestSelectedCellAtScrollPosition:UITableViewScrollPositionTop animated:YES];
}
